...
export default function ReviewView({ topic, onComplete, onBack }: ReviewViewProps) {
  const initialQuestions = useMemo(() => generateMCQuestions(topic, GRAPH, REVIEW_COUNT), [topic]);
  ...
  }, [onComplete, totalCorrect, totalAnswered, missedTopics]);
    ...
    return (
      <div style={containerStyle}>
        <div style={{ ...cardStyle, textAlign: "center" as const }}>
          ...
          <h2 style={{ fontFamily: "Georgia, 'Times New Roman', serif", fontSize: "1.5rem", fontWeight: 600, color: "var(--text-primary)", marginBottom: "0.5rem" }}>
            Review Complete!
          </h2>
          <div style={{ fontFamily: "'Noto Serif SC', serif", fontSize: "2rem", color: "var(--text-primary)", margin: "0.75rem 0" }}>
            {topic.hanzi}
          </div>
          ...
          <div style={{ fontFamily: "Georgia, 'Times New Roman', serif", fontSize: "13px", color: "var(--text-muted)", marginTop: "1rem" }}>
            {totalCorrect}/{totalAnswered} correct
          </div>
          {missedTopics.length > 0 && (
            <div style={{ fontFamily: "Georgia, 'Times New Roman', serif", fontSize: "13px", color: "var(--error)", marginTop: "0.5rem" }}>
              {missedTopics.length} topic{missedTopics.length > 1 ? "s" : ""} flagged for re-review
            </div>
          )}
    ...
    <div style={containerStyle}>
      ...
      <div style={cardStyle}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1rem" }}>
          <span style={{ ...badgeStyle, background: "var(--text-muted)" }}>Review</span>
